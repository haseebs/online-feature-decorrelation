  ...
  if (my_experiment.get_int_param("sum_features") && !my_experiment.get_int_param("n2_decorrelate"))
	...
	for (int step = 0; step < my_experiment.get_int_param("steps"); step++) {
		if (step % my_experiment.get_int_param("replace_every") == 1) {
			auto graphs = learning_network.replace_features(my_experiment.get_float_param("replace_perc"),
			                                                my_experiment.get_int_param("n2_decorrelate"),
			                                                my_experiment.get_int_param("sum_features"));
			int id = 0;
			for (const auto &graph : graphs) {
				...
				cur_graphs.push_back(std::to_string(id++));
