...
//DenseNetwork::DenseNetwork(float step_size,
//                           int seed,
...
DenseNetwork::DenseNetwork(float step_size,
	...
	this->second_mt.seed(seed+9999);
	this->third_mt.seed(seed+99999);
	std::uniform_real_distribution<float> weight_sampler(-10, 10);
	std::uniform_real_distribution<float> prob_sampler(0, 1);
	std::uniform_int_distribution<int> index_sampler(0, no_of_input_features-1);
    ...
    for (int j = 0; j < no_of_input_features; j++) {
      int new_index = j;
			float new_weight = weight_sampler(mt);
			Synapse *new_synapse = new Synapse(this->input_neurons[new_index],
			                                   new_feature,
...
std::string DenseNetwork::get_graph(int id1, int id2) {
	return "NA";
...
void DenseNetwork::replace_features_with_idx(int feature_idx) {
	std::uniform_real_distribution<float> weight_sampler(-1, 1);
	std::uniform_real_distribution<float> prob_sampler(0, 1);
...
float DenseNetwork::get_normalized_value(int idx){
	return (intermediate_neurons[idx]->value - feature_mean[idx]) / sqrt(feature_std[idx]);
}

void DenseNetwork::calculate_all_correlations() {
...
void DenseNetwork::update_random_correlation_selections_using_thresh(bool age_restriction){
...
//void DenseNetwork::update_random_correlation_selections(bool age_restriction){
//	std::vector<intpair > expired_pairs; //pairs that we gotta stop considering
//	//std::cout << "all pairs: " << std::endl;
//	for (auto & [id_pair, corr] : random_feature_correlations) {
//		//std::cout << intermediate_neurons[id_pair.first]->id << "\t->\t" << intermediate_neurons[id_pair.second]->id << "\t:\t" << feature_correlations[id_pair] << "\t age: " << intermediate_neurons[id_pair.first]->neuron_age << "-" << intermediate_neurons[id_pair.second]->neuron_age << std::endl;
...
//	}
//}
void DenseNetwork::update_random_correlation_selections(bool age_restriction, float perc_of_total_pairs_to_estimate){
...
void DenseNetwork::calculate_random_correlations(int min_estimation_period){
	//std::cout << "all pairs: " << std::endl;
...
std::vector<std::pair<floatpair, std::string> > DenseNetwork::replace_features_random_decorrelator_v3(float perc_to_replace, bool sum_features, float perc_to_decorrelate) {
...
std::vector<std::pair<floatpair, std::string> > DenseNetwork::replace_features_n2_decorrelator_v3(float perc_to_replace, bool sum_features, float perc_to_decorrelate) {
	std::vector<std::pair<intpair, float> > id_pair_correlations;
	std::vector<std::pair<floatpair, std::string> > correlated_graphviz;
	int max_replacements = int(prediction_weights.size() * perc_to_replace);
	int replaced_counter = 0;
...
void DenseNetwork::replace_features(float perc_to_replace) {
	int max_replacements = int(prediction_weights.size() * perc_to_replace);
	int replaced_counter = 0;
...
void DenseNetwork::replace_features_randomly(float perc_to_replace) {
...
float DenseNetwork::forward(std::vector<float> inputs) {
...
void DenseNetwork::print_all_correlations() {
...
int DenseNetwork::count_mature_features() {
	int counter = 0;
...
int DenseNetwork::count_highly_correlated_features() {
	//std::cout << "highly correlated idds: " << std::endl;
	std::set<int> correlated_feature_ids;
	for (int i = 0; i < intermediate_neurons.size(); i++) {
		for (int j = i+1; j < intermediate_neurons.size(); j++) {
			auto id_pair = std::make_pair(intermediate_neurons[i]->id, intermediate_neurons[j]->id);
			if (fabs(feature_correlations[id_pair]) > 0.85) {
...
void DenseNetwork::print_all_statistics() {
	for (int i = 0; i < intermediate_neurons.size(); i++) {
...
std::vector<float> DenseNetwork::get_feature_utilities() {
...
std::vector<float> DenseNetwork::get_prediction_weights() {
	std::vector<float> my_vec;
...
std::vector< std::pair < float, int >> DenseNetwork::get_prediction_weight_statistics(){
...
std::vector<float> DenseNetwork::get_prediction_gradients() {
	std::vector<float> my_vec;
...
void DenseNetwork::update_parameters_only_prediction(float error) {
...
void DenseNetwork::update_parameters_only_prediction(float error, float l2_lambda, float l1_lambda) {
	//TODO assumes single outgoing weight from interm neuron
	for (int index = 0; index < prediction_weights.size(); index++)
...
void DenseNetwork::update_parameters(float error) {
	//TODO assumes single outgoing weight from interm neuron
	for (int index = 0; index < intermediate_neurons.size(); index++) {
...
float DenseNetwork::read_output_values() {
